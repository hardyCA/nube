...
export default function EditarProductoPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    const { error: updateError } = await supabase.from('productos').update(productoData).eq('id', params.id)
  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      <h1 className="text-2xl font-bold tracking-tight mb-6">Editar producto</h1>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="bg-white rounded-xl border border-border p-6 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <Input label="Precio *" name="precio" type="number" step="0.01" min="0" value={form.precio} onChange={handleChange} required />
            <Input label="Precio de oferta" name="precio_oferta" type="number" step="0.01" min="0" value={form.precio_oferta} onChange={handleChange} />
          </div>
          <div className="grid grid-cols-3 gap-4">
            <Input label="Stock *" name="stock" type="number" min="0" value={form.stock} onChange={handleChange} required />
            <Input label="Stock mínimo" name="stock_minimo" type="number" min="0" value={form.stock_minimo} onChange={handleChange} />
            <Input label="Unidad" name="unidad_medida" value={form.unidad_medida} onChange={handleChange} />
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl border border-border p-6 space-y-4">
          ...
          <div>
            ...
            <div className="flex items-center gap-4">
              ...
              {form.imagen_url && (
                <button type="button" onClick={() => setForm(prev => ({ ...prev, imagen_url: '' }))} className="text-sm text-red-600 hover:underline">
                  Eliminar
                </button>
              )}
            </div>
        ...
        <div className="bg-white rounded-xl border border-border p-6 space-y-4">
          ...
          <label className="flex items-center gap-3 cursor-pointer">
            <input type="checkbox" name="destacado" checked={form.destacado} onChange={handleChange} className="w-4 h-4 rounded border-border" />
